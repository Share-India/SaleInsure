...
const TeamPerformance = ({ data }) => {
  ...
  const handlePasswordChange = async (userId, newPassword) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/users/${userId}/password?password=${encodeURIComponent(newPassword)}`, {
        method: 'PUT',
        headers: {
  ...
  const handleExport = (e) => {
    ...
    try {
        ...
        const headers = ['Salesperson', 'Role', 'Total Revenue', 'Total Deals', 'Connections', 'Conversations', 'Closures', 'Quota Status'];
        const csvRows = topPerformers.map(rep => {
            return [
                `"${(rep.username || '').replace(/"/g, '""')}"`,
                `"${(rep.role || '').replace(/"/g, '""')}"`,
                rep.totalRevenue || 0,
                rep.totalDeals || 0,
                rep.connections || 0,
        ...
        if (window.navigator && window.navigator.msSaveOrOpenBlob) {
            window.navigator.msSaveOrOpenBlob(blob, 'team_performance_export.csv');
            return;
        }
        ...
        link.setAttribute("download", `team_performance_export_${new Date().toISOString().split('T')[0]}.csv`);
      ...
      <main className="max-w-7xl mx-auto px-8 py-16 space-y-12">
        <header className="flex flex-col md:flex-row md:items-end justify-between gap-6 border-b border-outline-variant/30 pb-10">
          ...
          <div className="flex items-center gap-4">
            <button type="button" onClick={handleExport} className="px-6 py-2.5 bg-primary-container text-on-primary font-label-caps uppercase tracking-widest rounded-lg shadow-lg hover:opacity-90 transition-all flex items-center gap-2">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            {rank2 && (
              <div 
                ...
                className="order-2 md:order-1 bg-surface-container-lowest border border-outline-variant/40 rounded-xl p-8 shadow-sm relative overflow-hidden group transition-transform hover:-translate-y-1 cursor-pointer"
              >
                <div className="absolute top-0 right-0 p-4">
                  <span className="text-blue-900/10 font-h1 text-7xl select-none">02</span>
                </div>
                <div className="relative z-10 space-y-6">
                    <div className="w-16 h-16 rounded-full bg-slate-200 border-4 border-white shadow-sm flex items-center justify-center overflow-hidden">
                      {rank2.profilePhotoUrl ? (
                          <img src={`${API_BASE_URL}${rank2.profilePhotoUrl}`} alt={rank2.username} className="w-full h-full object-cover" />
                      ) : (
                          <span className="text-xl font-bold text-slate-500">{rank2.username.substring(0, 2).toUpperCase()}</span>
                      )}
                    </div>
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-outline-variant/20">
                    <div>
                      <p className="font-label-caps text-on-surface-variant opacity-60 mb-2">TOTAL REVENUE</p>
                      <p className="font-data-tabular text-2xl font-black text-primary-container">{formatCurrency(rank2.totalPremium)}</p>
                    </div>
                    <div>
                      <p className="font-label-caps text-on-surface-variant opacity-60 mb-2">QUOTA STATUS</p>
                      <p className="font-data-tabular text-2xl font-black text-primary-container">{rank2.quota ? Math.round((rank2.totalPremium / rank2.quota) * 100) : 0}%</p>
                    </div>
                  </div>
                  <div className="space-y-3 pt-2">
                    <div className="flex justify-between font-label-caps text-xs text-on-surface-variant opacity-80">
                      <span>Monthly Progress</span>
                      <span>{formatCurrency(rank2.quota)} Goal</span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min(100, (rank2.totalPremium / (rank2.quota || 1)) * 100)}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
            ...
            {mvp && (
              <div 
                onClick={() => setSelectedUser(mvp)}
                className="order-1 md:order-2 bg-blue-600 text-white border border-blue-500 rounded-xl p-10 shadow-xl relative overflow-hidden group scale-105 z-20 transition-transform hover:-translate-y-2 cursor-pointer"
              >
                ...
                <div className="relative z-10 space-y-8">
                  <div className="flex justify-between items-start">
                    <div className="w-28 h-28 rounded-full bg-blue-800 border-4 border-white/20 shadow-xl flex items-center justify-center overflow-hidden">
                      {mvp.profilePhotoUrl ? (
                          <img src={`${API_BASE_URL}${mvp.profilePhotoUrl}`} alt={mvp.username} className="w-full h-full object-cover" />
                      ) : (
            ...
            {rank3 && (
              <div 
                ...
                className="order-3 bg-surface-container-lowest border border-outline-variant/40 rounded-xl p-8 shadow-sm relative overflow-hidden group transition-transform hover:-translate-y-1 cursor-pointer"
              >
                <div className="absolute top-0 right-0 p-4">
                ...
                <div className="relative z-10 space-y-6">
                    <div className="w-16 h-16 rounded-full bg-slate-200 border-4 border-white shadow-sm flex items-center justify-center overflow-hidden">
                      {rank3.profilePhotoUrl ? (
                          <img src={`${API_BASE_URL}${rank3.profilePhotoUrl}`} alt={rank3.username} className="w-full h-full object-cover" />
                      ) : (
                          <span className="text-xl font-bold text-slate-500">{rank3.username.substring(0, 2).toUpperCase()}</span>
                      )}
                    </div>
                  <div className="space-y-1">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 pt-4 border-t border-outline-variant/20">
                    <div>
                      <p className="font-label-caps text-on-surface-variant opacity-60 mb-2">TOTAL REVENUE</p>
                      <p className="font-data-tabular text-2xl font-black text-primary-container">{formatCurrency(rank3.totalPremium)}</p>
                    </div>
                    <div>
                      <p className="font-label-caps text-on-surface-variant opacity-60 mb-2">QUOTA STATUS</p>
                      <p className="font-data-tabular text-2xl font-black text-primary-container">{rank3.quota ? Math.round((rank3.totalPremium / rank3.quota) * 100) : 0}%</p>
                    </div>
                  </div>
                  <div className="space-y-3 pt-2">
                    <div className="flex justify-between font-label-caps text-xs text-on-surface-variant opacity-80">
                      <span>Monthly Progress</span>
                      <span>{formatCurrency(rank3.quota)} Goal</span>
                    </div>
                    <div className="h-1.5 w-full bg-slate-200 rounded-full overflow-hidden">
                      <div className="h-full bg-blue-500 rounded-full" style={{ width: `${Math.min(100, (rank3.totalPremium / (rank3.quota || 1)) * 100)}%` }}></div>
                    </div>
                  </div>
                </div>
              </div>
              ...
              {divisionReps.map((rep, index) => {
                ...
                return (
                  <div 
                    ...
                    className="bg-white border border-outline-variant/40 rounded-xl p-6 shadow-sm hover:shadow-md transition-all cursor-pointer"
                  >
                    <div className="flex items-center gap-4 mb-6">
                      <div className="w-10 h-10 rounded-full border border-outline-variant overflow-hidden bg-slate-100 flex items-center justify-center shrink-0">
                        {rep.profilePhotoUrl ? (
                            <img src={`${API_BASE_URL}${rep.profilePhotoUrl}`} alt={rep.username} className="w-full h-full object-cover" />
                        ) : (
                            <span className="text-sm font-bold text-slate-600">{rep.username.substring(0, 2).toUpperCase()}</span>
                        )}
                      </div>
                      <div>
